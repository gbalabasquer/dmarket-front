...
export function doHashChange() {
  ...
  localStorage.setItem('quoteToken', 'MKR');
  ...
  if (location.hash.indexOf('#deposit') === -1 && location.hash.indexOf('#transfer') === -1) {
    if (location.hash.indexOf('#trade') === -1) {
      location.hash = `#trade/${localStorage.getItem('baseToken') || 'MKR'}`;
    }
    const coins = location.hash.replace('#trade/', '').split('/');
    if (coins.length === 2) {
      quoteToken = coins[0].toUpperCase();
      baseToken = coins[1].toUpperCase();
    }
  }

  doTabShow();

  Session.set('quoteToken', quoteToken || localStorage.getItem('quoteToken') || 'MKR');
  //Session.set('baseToken', baseToken || localStorage.getItem('baseToken') || 'W-ETH');
}
